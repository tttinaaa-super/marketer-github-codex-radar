...
def score_repo(repo: dict[str, Any], now: datetime) -> Row | None:
    ...
    adoption = math.log10(stars + 10) * 8.0 + math.log10(forks + 5) * 4.0
    relevance = sum(categories.values()) * 7.0
    fit = codex_fit(text, repo)
    fit_bonus = {"install": 18.0, "wrap": 10.0, "reference": 4.0, "skip": -20.0}[fit]
    score = relevance + adoption + freshness * 20.0 + fit_bonus - coding_penalty * 8.0
...
def render(rows: list[Row]) -> str:
    out = [
        "| Rank | Repository | Use | Codex fit | Stars | Forks | Updated | Why it matters |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | --- |",
    ]
    for rank, row in enumerate(rows, 1):
        repo = f"[{row.repo}]({row.url})" if row.url else row.repo
        out.append(
            f"| {rank} | {repo} | {row.use} | {row.fit} | {row.stars} | "
            f"{row.forks} | {row.updated_days:.1f}d | {row.why} |"
        )
    return "\n".join(out)
